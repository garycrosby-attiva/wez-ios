# spotted-capture Specification

## Purpose

Defines the Spotted capture flow: a focused, live-camera-only modal task over the Spotted
tab that captures a photo, GPS auto-tags it to a seeded beach for confirmation, presents an
inert join-me alert step, and persists the resulting Ez-authored post and photo to the local
backend.
